...
// --- Todos os styled-components existentes permanecem os mesmos ---
...
// ✅ NOVOS COMPONENTES ESTILIZADOS PARA O LAYOUT
...
const Automate = () => {
  // --- Lógica de estado e funções permanecem as mesmas ---
  ...
  // Hook de efeito para extrair chaves e atualizar o conteúdo substituído
  useEffect(() => {
    // Extrai chaves do conteúdo principal
    const extractedKeys = extractTextKeys(content);
    
    // Atualiza a lista de chaves, preservando valores existentes
    ...
    // Recalcula o conteúdo substituído sempre que as chaves ou seus valores mudam
    let result = content;
  ...
  return (
    <Container>
      ...
      {/* ✅ ESTRUTURA JSX MODIFICADA PARA O LAYOUT DE 2 COLUNAS */}
      <MainLayout>
        <LeftColumn>
          <EditorContainer>
            <EditorTitle>📝 Editor Principal</EditorTitle>
            <Editor
              apiKey="SUA_CHAVE_API_AQUI" // Lembre-se de usar sua chave de API do TinyMCE
              value={content}
          ...
          <PreviewContainer hasContent={!!(replacedContent || content)}>
            ...
            <Editor
              apiKey="SUA_CHAVE_API_AQUI" // Use a mesma chave de API
        ...
        <RightColumn>
          {content.trim() && (
           ...
           <InfoSection>
            ...
            <InfoText>
              <strong>1. Crie o Modelo:</strong> Digite no editor e use chaves duplas para placeholders, como `{{nome}}` ou `{{produto}}`.
            </InfoText>
            <InfoText>
              <strong>2. Preencha os Valores:</strong> As chaves aparecerão aqui. Preencha os campos para cada uma delas.
            </InfoText>
            <InfoText>
              <strong>3. Visualize e Exporte:</strong> A pré-visualização é atualizada em tempo real. Ao terminar, clique para obter e copiar seu conteúdo final em HTML ou texto.
            </InfoText>
          </InfoSection>
        </RightColumn>
      </MainLayout>
