Com certeza\! Vamos atualizar o código para usar `||key-name||` como o novo delimitador em vez de `{{key-name}}`.

Isso exigirá a alteração em três áreas principais:

1.  **Lógica de Extração:** A função `extractTextKeys` precisa de uma nova expressão regular (regex) para encontrar o padrão `||...||`.
2.  **Lógica de Substituição:** A expressão regular usada para substituir as chaves pelo seu valor também precisa ser atualizada.
3.  **Interface (UI):** O texto de exemplo e os rótulos que mostram o formato da chave para o usuário devem ser modificados.

Abaixo está o código completo com todas as alterações necessárias.
